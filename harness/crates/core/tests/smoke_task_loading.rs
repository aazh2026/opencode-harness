...
fn get_smoke_api_001_path() -> PathBuf {
    let mut path = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
    path.push("../../../harness/tasks/api/SMOKE-API-001.yaml");
    path
}
...
#[test]
fn smoke_api_001_yaml_parses_correctly() {
    let path = get_smoke_api_001_path();
    let content = std::fs::read_to_string(&path).unwrap_or_else(|e| {
        panic!(
            "Failed to read SMOKE-API-001.yaml: {} (path: {:?})",
            e, path
        )
    });
    let task: Task =
        serde_yaml::from_str(&content).expect("SMOKE-API-001.yaml should parse as valid Task YAML");

    assert_eq!(task.id, "SMOKE-API-001");
    assert_eq!(task.title, "API health endpoint returns status");
    assert_eq!(task.category, TaskCategory::Smoke);
    assert_eq!(task.fixture_project, "fixtures/projects/api-project");
    assert_eq!(task.preconditions, vec!["opencode API server running"]);
    assert_eq!(task.entry_mode, EntryMode::API);
    assert_eq!(task.agent_mode, AgentMode::Batch);
    assert_eq!(task.provider_mode, ProviderMode::Both);
    assert_eq!(task.input.command, "opencode");
    assert_eq!(task.input.args, vec!["api", "health"]);
    assert_eq!(task.severity, Severity::Critical);
    assert_eq!(task.tags, vec!["smoke", "api", "health"]);
    assert_eq!(task.timeout_seconds, 30);
}

#[test]
fn smoke_api_001_schema_validation_passes() {
    let validator = DefaultTaskSchemaValidator::new();
    let path = get_smoke_api_001_path();
    let content = std::fs::read_to_string(&path).unwrap();
    let task: Task = serde_yaml::from_str(&content).unwrap();
    ...
    assert!(
        result.is_ok(),
        "SMOKE-API-001 should pass schema validation: {:?}\n\
         NOTE: ID format follows '^[A-Z]+-[A-Z]+-[0-9]+$' pattern (e.g., SMOKE-API-001)",
        result
    );
}

#[test]
fn smoke_api_001_has_correct_assertions() {
    let path = get_smoke_api_001_path();
    let content = std::fs::read_to_string(&path).unwrap();
    let task: Task = serde_yaml::from_str(&content).unwrap();
    ...
    assert!(task
        .expected_assertions
        .contains(&AssertionType::StdoutContains("healthy".to_string())));
}

#[test]
fn smoke_api_001_task_loader_can_load_task() {
    let path = get_smoke_api_001_path();
    let content = std::fs::read_to_string(&path).unwrap();
    let task: Task = serde_yaml::from_str(&content).unwrap();
